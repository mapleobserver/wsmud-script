...
[if] (FBWay) == 扫荡
    //扫荡模式
    [if] (FBName) == 青城山(第三方流程) || (FBName) == 云梦沼泽(第三方流程) || (FBName) == 白驼山(组队) || (FBName) == 冰火岛(偷渡) || (FBName) == 移花宫(偷渡) || (FBName) == 燕子坞(偷书) || (FBName) == 缥缈峰(偷渡) || (FBName) == 光明顶(组队) || (FBName) == 光明顶(偷渡) || (FBName) == 天龙寺(偷渡) || (FBName) == 古墓派(偷渡)
...
[else if] (FBWay) == 自动
    ...
    [else if] (FBName) == 云梦沼泽(第三方流程)
        @call 云梦沼泽 ,,,20
    [else if] (FBName) == 青城山(第三方流程)
        @call 青城山 ,,,20
