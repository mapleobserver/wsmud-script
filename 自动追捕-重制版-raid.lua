...
[while](currentN) < 20
    ...
    [while] (escape) == null
        ask1 {程药发}
        @toolbar tasks
        @task 追杀逃犯：($escape)，据说最近在($dir1)-($dir2)出现过，你还有($min)分($sec)秒去寻找他，目前完成($currentN)/20个，共连续完成($comboN)个。|追杀逃犯：目前完成($currentN)/20个，共连续完成($comboN)个
    $wait 500
    [while] {(escape)}? == null
        <---
        @cmdDelay 1000
        [if] {(escape)}? != null
            ($type1) = null
            kill {(escape)}
            @until {(escape)的尸体}? != null | {r(escape)}? == null | (:combating) == false
